use sfml::{graphics::{RenderTarget, RenderWindow, Sprite, Texture, Transformable}, system::{Vector2f}};
use processor::CPU;
use crate::processor;
        ...
        while i < (n + 1) as u64 {
            ...
            while j < 8 * 4 {
               // if cpu.ram[(i * 32 + j + cpu.ptr as u64) as usize] == 1 {
                    ...
                    self.pixels[(((i + x as u64) * 64) * 4 + (j + y as u64) * 4 + 1) as usize] = 255;
                    self.pixels[(((i + x as u64) * 64) * 4 + (j + y as u64) * 4 + 2) as usize] = 0;
                    self.pixels[(((i + x as u64) * 64) * 4 + (j + y as u64) * 4 + 3) as usize] = 255;
                // }
                // else {
